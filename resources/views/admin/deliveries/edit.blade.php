...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Редактировать способ доставки</h1>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Редактирование - {{ $delivery->title }}</h3>
                        </div>
                        <!-- /.card-header -->

                        <form role="form" method="post" action="{{ route('deliveries.update', ['delivery' => $delivery->id]) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                                ...
                                                <div class="tab-pane fade @if($k === 1) active show @endif" id="tabs-{{ $k }}" role="tabpanel">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="regions[{{ $k }}][add]" id="delivery_{{ $k }}" class="form-control" data-placeholder="Добавить регион?">
                                                            <option value="1" @if($k == $delivery->region->contains($k)) selected @endif>Да</option>
                                                            <option value="0" @if(!$k == $delivery->region->contains($k)) selected @endif>Нет</option>
                                                        </select>
                                                    </div>
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="regions[{{$k}}][days]"
                                                               class="form-control @error('title') is-invalid @enderror"
                                                                value = "@if($k == $delivery->region->contains($k)) {{ $delivery->region->firstWhere('id', $k)->pivot->days }}  @endif">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="days">Стоимость в рублях</label>
                                                        <input type="text" name="regions[{{$k}}][value]"
                                                               class="form-control @error('title') is-invalid @enderror"
                                                               value = "@if($k == $delivery->region->contains($k)) {{ $delivery->region->firstWhere('id', $k)->pivot->value }}  @endif">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="days">Стоимость в процентах <small>(через точку)</small></label>
                                                        <input type="text" name="regions[{{$k}}][percent]"
                                                               class="form-control @error('title') is-invalid @enderror"
                                                               value = "@if($k == $delivery->region->contains($k)) {{ $delivery->region->firstWhere('id', $k)->pivot->percent }}  @endif">
                                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea type="text" name="not_regions"
                                           class="form-control" id="not_regions" rows="5">{{ $not_regions }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>Статус</label>
                                    <select class="form-control" name="status" id="status" data-placeholder="Выберите статус" autocomplete="off">
                                        <option value="1" @if($delivery->status) selected @endif>Включен</option>
                                        <option value="0" @if(!$delivery->status) selected @endif>Отключен</option>
                                    </select>
                                </div>
                            </div>
                            <!-- /.card-body -->
